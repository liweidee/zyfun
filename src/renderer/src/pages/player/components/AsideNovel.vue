<template>
  <div class="container-aside-wrap container-aside-novel">
    <div v-if="!active.profile" class="container-wrap main-wrap">
      ...
      <div class="anthology-container novel-anthology">
...
const emits = defineEmits(['update', 'barrage', 'create', 'pause', 'seek', 'novel-content-change']);
...
// 解析小说播放结果
const parseNovelPlayResult = (url: string): { title: string; content: string } | null => {
  if (url && url.startsWith('novel://')) {
    try {
      const jsonStr = decodeURIComponent(url.substring(8));
...
const handleSwitchSeason = async (item: ICmsInfoEpisode, index: number = -1) => {
  ...
  // 保存当前章节列表
  let chapterList: { text: string; link: string }[] = [];
  const episodes = infoConf.value.vod_episode?.[currentLine];
  ...
  try {
    ...
    const novelData = parseNovelPlayResult(playRes.url);
    if (!novelData) throw new Error('解析小说内容失败');

    const foundIndex = chapterList.findIndex((ch) => ch.link === item.link);
    const chapterIndex = foundIndex !== -1 ? foundIndex : index >= 0 ? index : 0;

    // 发送内容到父组件显示
    emits('novel-content-change', {
      title: novelData.title || item.text,
    ...
    videoData.value.duration = 1;
    videoData.value.playEnd = false;
  } catch (error) {
    console.error('[Novel] 加载失败:', error);
    MessagePlugin.error('加载小说内容失败，请稍后重试');
  }
};
...
// 暴露方法给父组件调用（用于上一章/下一章/章节选择）
const loadChapter = async (item: ICmsInfoEpisode, index: number) => {
  await handleSwitchSeason(item, index);
};

defineExpose({
  loadChapter,
  handleSwitchSeason, // 同时暴露，方便调试
});

emitter.on(emitterChannel.COMP_MULTI_PLAYER_PLAYNEXT, async () => {});
</script>
<style lang="less" scoped>
.container-aside-novel {
  display: flex;
